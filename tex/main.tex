...
we want to compare our model to observations. Our model is constructed using 
the SMF and SMHMR at $z = 1$ and based on the observed evolution of the SFMS. 
It has two free parameters $A_z$ and $m_z$, which allow for a flexible redshift 
dependence of the SFMS. At the final snapshot $z \sim 0$, besides the SFR distribution,
which our model reproduces by construction, we want our model to reproduce both 
the observed SMF and the scatter in SMHMR. 


\bitem
\item brief paragraph about inference using ABC
\eitem
