...
SHAM we have the initial $M_*$s of our SF centrals at $z \sim 1$. Their
initial SFRs, we can assign based on observed SFR-$M_*$ relation of the 
SFMS. The SFMS in the literature, however, have significant discrepancies. 
In \cite{speagle2014}, 
they compile SFRs of the SFMS from 25 studies in the literature, each 
with different methods of deriving galaxy SFR and $M_*$. Even after 
the calibration, the SFRs of the SFMS from different analyses at $z \sim 1$ 
for $M_* = 10^{10.5}\, M_\odot$ vary by more than a factor of 
2~\citep[see Figure 2 of][]{speagle2014}. This is particularly concerning 
since the amplitude and slope of the SFMS significantly dictate the SFR 
and $M_*$ evolution of SF centrals in our model. In order to deal with 
this uncertainty, we use the following prescription for the SFMS SFR: 
$\log\,\mathrm{SFR}_\mathrm{MS}(M_*, z)$. 

At $z \sim 0$, the central galaxies of the SDSS DR7 group catalog 
provide an anchoring measurement of the SFMS. We therefore fix 
$\log\,\mathrm{SFR}_\mathrm{MS}(M_*, z=0)$ to $\log\, \mathrm{SFR}(M_*)$ 
of the SDSS central galaxy SFMS. To obtain the SFMS $\log\, \mathrm{SFR}(M_*)$ 
of the SDSS centrals, we again take the SFMS GMM components, used to 
estimate $f^\mathrm{cen}_\mathrm{SFMS}$, at each stellar mass bin and 
linearly fit their mean $\log\, \mathrm{SFR}$ as a function of $M_*$. 
...
$A_z$ and $m_z$, respectively. 
\beq
\log\,\mathrm{SFR}_\mathrm{MS}(M_*, z) = 
\eeq
Later in our analysis, we set the following priors for $A_z$ and $m_z$: 
\todo{prior}. These priors are conservatively chosen to encompass the 
best-fit SFMS from \cite{speagle2014} and also measurements from 
\cite{moustakas2013,lee2015}. 
