...
describe how we derive this $f^\mathrm{cen}_\mathrm{SFMS}(M_*)$ and use 
...
galaxies in the ``green valley''. For our $f^\mathrm{cen}_\mathrm{SFMS}(M_*)$, 
we instead use a method from Hahn et al.~(in prep) and Tjitske et 
al.~(in prep), which is a data-driven fitting of the SFS from the SFR-$M_*$ 
distribution using Gaussian Mixture Models (GMMs). 

\todo{shorten: emphasize the advantages}
...
Next, using the $f^\mathrm{cen}_\mathrm{SFMS}$ spanning the different 
stellar mass bins, we fit $f^\mathrm{cen}_\mathrm{SFMS}$ as 
...
\includegraphics[width=0.6\textwidth]{figs/illustris_sfh.pdf} 
    ...
    centrals in Section~\ref{sec:sfcen}. The $\Delta \logsfr(t)$ of these Illustris 
    star-forming galaxies stochastically fluctuate about the mean SFMS. Our prescription  
    for the SFHs of the SF centrals in our model (Section~\ref{sec:modelevol}) is based 
    on this $\Delta \logsfr(t)$ behavior.}
\label{fig:illsfh}
\end{center}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
model. Below, we describe our specific prescription for $\Delta \logsfr(t)$.
...
$z \sim 1$. One naive prescription for $\Delta \logsfr(t)$, would be to 
fix $\Delta \logsfr$ based on the initial offsets from the $\musfms$. SF centrals 
...
motivation for constant $\Delta \logsfr(t)$. In Figure~\ref{fig:illsfh}, we present 
$\Delta \logsfr$ of star-forming Illustris galaxies as a function of cosmic time. 
These galaxies have stellar masses within $10^{10.5}-10^{10.6}M_\odot$ at $z=0$. 
At the simultation outputs, we derive $\musfms$ using the same fitting method 
we used for the SDSS centrals in Section~\ref{sec:sfcen} and use it to calculate
$\Delta \logsfr$ (Eq.~\ref{eq:logsfr_sf}). Rather than remaining constant, 
Figure~\ref{fig:illsfh} illustrates that the $\Delta \logsfr(t)$s stochastically 
vary about SFMS. 

Motivated by the $\Delta \logsfr(t)$ of Illustris galaxies, we introduce the 
``\emph{star formation duty cycle}'' to our $\Delta \logsfr(t)$. We parameterize 
$\Delta \logsfr$ to randomly flucutate about the mean SFMS on some duty cycle 
timescale $t_\mathrm{duty}$. Then the SFR of a given SF central in our model 
will follow Eq.~\ref{eq:logsfr_sf} where at every timestep $t_\mathrm{duty}$, 
$\Delta \logsfr$ is randomly sampled from a log-normal distribution with 
$0.3\,\mathrm{dex}$ scatter. In the left panel of Figure~\ref{fig:sfh_model}, we 
present $\Delta \logsfr(t)$ of SF centrals in our model with our star formation 
...
$0.3\,\mathrm{dex}$ $1\sigma$ scatter of the SFMS SFR. \todo{Such a simplified model 
obviously does not reflect individual SFHs of SF galaxies; however, since 
$\Delta \logsfr$ is randomly sampled, we expect it capture the stochasticity 
from gas accretion and star-bursts.} Furthermore, this prescription of $\Delta \logsfr$ 
produces SFHs that reproduce the log-normal SFR distribution of the SFMS 
...
models by construction, reproduce the $z\sim0$ SFMS. Lastly, on the right, 
...
%Now that we have a model that evolves the SFRs and $M_*$s of SF centrals, we can compare this model to observations. Our model is constructed using the SMF and SMHMR at $z = 1$ and based on the observed evolution of the SFMS.  It has two free parameters $A_z$ and $m_z$, which allow for a flexible redshift dependence of the SFMS. At the final snapshot, $z \sim 0$, our model reproduces the observed SFR distribution by construction. However, we also want our model to 
