...
we compare our model to observations. Our model is constructed using the SMF 
...
the SFR distribution by construction. Therefore, we compare our model the SMF of our 
model at $z \sim 0$ to the SDSS DR7 VAGC SMF from \cite{li2009}. For the parameter 
estimation, we use Approximate Bayesian Computation (ABC). ABC has the advantage over standard approaches to parameter inference in that it does not 
...
however we use ABC in conjunction with the efficient Population Monte Carlo (PMC)
importance sampling as in~\citep{hahn2016,hahn2017}. 
