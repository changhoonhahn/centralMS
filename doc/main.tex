...
\includegraphics[width=0.75\textwidth]{figs/SHMRscatter_tduty.pdf}
    \caption{Our models with shorter star formation duty cycle timescales, $t_\mathrm{duty}$, 
    produce smaller scatter in the SHMR: $\sigma_{\log\,M_*}$ at $M_h{=}10^{12} M_\odot$ (left) 
    and $\sigma_{\log\,M_h}$ at $M_*{=}10^{10} M_\odot$. For $t_\mathrm{duty} = 10\,\mathrm{Gyr}$, 
    $\sigma_{\log\,M_*} = 0.32$ and $\sigma_{\log\,M_h} = 0.30$. Meanwhile for $
    t_\mathrm{duty} = 0.5\,\mathrm{Gyr}$, $\sigma_{\log\,M_*}= 0.26$ and 
    $\sigma_{\log\,M_h} = 0.20$. We include $\sigma_{\log\,M_*}$ 
...
has \todo{no star formation duty cycle} (red) while the other has a duty cycle 
of $t_\mathrm{duty} = 1\,\mathrm{Gyr}$ (blue). Both models successfully reproduce 
the SDSS SMF, $\Phi^\mathrm{SDSS}_\mathrm{SF,cen}$. By construction, they also 
produce SFSs that are consistent with observations based on the priors. \todo{something something}

, \emph{the difference in the star formation duty cycle timescales of the models 
results in dramatically different scatter in $\log\,M_*$ for a given $
\log\,M_\mathrm{halo}$ (\emph{i.e.} scatter in the SHMR)}, particularly at
$M_\mathrm{halo} < 10^{12.5}M_\odot$. 

In other words the scatter in the SHMRs of our models with different timescale 
star formation duty cycles can be compared to observational constraints on the 
SHMR in order to constrain the star formation duty cycle and the SFHs of star 
...
as a function of their star formation duty cycle timescale in Figure~\ref{fig:sigMstar_duty}.
...
from weak lensing:
\bitem
\item \cite{mandelbaum2006a} observations constraints from SDSS galaxy–galaxy weak lensing, with a total of 351 507
    lenses. We use stellar masses derived from spectroscopy and virial halo masses derived from
    weak gravitational lensing.
    At $M_* = 1.5 \times 10^10 M_\odot$ $M_\mathrm{halo} = 6.6\substack{+5.1\\ -4.0}\times 10^{11}h^{-1} M_\odot$. 
...
\item \cite{velander2013}: 
    154 deg2 CFHTLenS lensing and photometric data using a galaxy-galaxy lensing halo model which 
    allows us to constrain the halo mass and satellite fraction. There's some scatter correction 
    which reduces the uncertainties significantly. At $< M_*> = 0.54 \times 10^10 M_\odot/h^2$ 
    $M_\mathrm{halo} = 2\substack{+0.64\\ -0.62}\times 10^{11}h^{-1} M_\odot$. 

\item \cite{han2015}: weak lensing analysis of optically selected spectroscopic galaxy 
    groups (G3Cv5) in GAMA survey using background SDSS photometric galaxies.
