...
with a correlation coefficient $r$, a parameter added to our model. We note that 
this prescription for correlating $\Delta\log\,\mathrm{SFR}$ to $\Delta M_h$
is similar to the~\cite{watson2015,rodriguez-puebla2016,behroozi2018} empirical 
models. In Figure~\ref{fig:mhacc_dsfr} we illustrate this prescription for galaxy
...
[$t$, $t - t_\mathrm{dyn}$]. Halo accretion over this period, $\Delta M_h = M_h(t) - M_h(t-t_\mathrm{dyn})$, 
correlates with $\Delta\logsfr(t)$. The SFHs in the middle and bottom panels 
illustrate this correlation and how $\Delta\logsfr(t)$ correlates more strongly 
with $\Delta M_h(t)$ for our model with higher $r$. 

Next, we repeat our analysis of inferring model parameters by comparing to  
...
our model with $r > 0.5$ and $t_{\rm duty} < 2$ Gyr is in tension with 
the Cao et al. (in preparation) constraint. 

In addition to the observational constraints, we compare our updated model 
to predictions from galaxy formation models included in Figure~\ref{fig:sigMstar_duty}.
...
$t_\mathrm{duty} < 1$ Gyr and $r > 0.5$, our model is in agreement with 
