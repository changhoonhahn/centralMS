...
\begin{abstract}
    ...
    accretion histories of star-forming central galaxies over $z < 1$. We then 
    compare these models to the observed stellar mass function and SFS of central 
    galaxies in SDSS Data Release 7 to find that the scatter in SHMR at 
    $M_h{=}10^{12}M_\odot$, $\sigma_{\log M_*}$, is sensitive to the
    ...
    $r$ also produce tighter $\sigma_{\log M_*}$. 
    \edt{We find $r > 0.5$ and $t_{\rm duty} \leq 1$ Gyr is necessary to reproduce 
    the constant $\siglogm \sim 0.2$ dex over $z=1$ to 0, as found in observed halo 
    model constraints and some hydrodynamic simulations. Meanwhile, to produce 
    $\siglogm$ that decreases from $z=1$ to 0, as found in Illustris TNG, 
    \ch{constraints} are necessary. 
    } 
    %For $r \sim 0.6$ constraints found in the literature, $t_{\rm duty} < 0.5$ Gyr is necessary. 
