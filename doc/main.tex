...
snapshot of our model. For the SF centrals and their host subhalos in our 
model, not only do we have SFRs and $M_*$s that we evolved but we also 
have halo histories from the $\mathtt{TreePM}$ $N$-body simulation. With 
these properties, we can compare our model to observations using standard 
observables such as the SMF. Since we have host halo histories, we can also 
use observables such as the SHMR. Next, we present the comparison between 
our model and observations and the constraints we derive on the role and 
timescale of star formation variability in SF galaxies.

\section{Results}
Our model takes central subhalos from $\mathtt{TreePM}$ and tracks their 
$SFR$ and $M_*$ evolution using a flexible parameterization of the SFS and 
SFHs that incorporate variability through a star formation duty cycle 
(Section~\ref{sec:modelevol}). At $z=0.05$, our model provides SFR, $M_*$, 
and host halo mass, $M_h$, of its SF central galaxies. We now use these 
properties to compare our model to observations. The parameterization of 
the SFS 


Although the SFS is parameterized using the free
parameters $m_{M_*}$ and $m_z$ and hence the SFR distribution cannot be used as an observable, 

the smf is a baller observable to compare to. 
