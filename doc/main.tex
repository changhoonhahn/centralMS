...
%{\ch instead talk about how wy we don't think this is actually the case using Jeremy's papers} 
%However, as the lack of evolution in the observed SHMR scatter and comparison from the previous section suggest, the SFR and $M_*$ evolution of star-forming centrals in our model is likely correlated with the accretion histories of their host halos. 
...
timescales $\leq 0.5$ Gyr is necessary to produce $\sigma_{\log\,M_*} \sim 0.2$ dex
as found in~\cite{more2011, leauthaud2012, reddick2013, tinker2013, zu2015} 
and hydrodynamic simulations. We note that this timescale is consistent 
with the timescale $\sim 0.5$ Gyr that \cite{sparre2015} find in Illustris
galaxies using a Principle Component Analysis of the SFHs. However, the 
$\sigma_{\log\,M_*}$ constraint of Cao et al. (in preparation) and 
predictions from SAMs and {\sc UniverseMachine} require a less stringent 
restriction on $t_\mathrm{duty}$: $< 5$ Gyr. 
...
%However, even with assembly bias a short duty 
...
{\ch paragraph that emphasizes the motivations}  
In order to measure the timescale of star formation variability and the connection 
between star formation and host halo accretion histories of star-forming central
galaxies. 
...
introduces variability on some specific timescale, $t_\mathrm{duty}$. We parameterize 
...
\item A shorter star formation duty cycle in our model produces significantly 
    tighter scatter in the SHMR, $\sigma_{\log\,M_*}$, at $M_h = 10^{12} M_\odot$. 
    ...
    the scatter in SHMR can constrain $t_\mathrm{duty}$, which represents the timescale 
...
\item We compare the $\sigma_{\log\,M_*}(M_h\sim10^{12}M_\odot)$ predicted by our 
    ...
    satellite kinematics, and galaxy-galaxy weak lensing observations. Although 
    there is tension among the constraints, observations generally favor a shorter 
    ...
    $\sigma_{\log\,M_*}$ at $M_h>10^{12}M_\odot$, short $t_\mathrm{duty} \lesssim 0.5$ Gyr 
    variations in SFH is necessary.
...
\item We next incorporate assembly bias in our model by correlating the star 
    ...
    $t_\mathrm{duty} < 1$ Gyr are necssary. 
    predictions from 
